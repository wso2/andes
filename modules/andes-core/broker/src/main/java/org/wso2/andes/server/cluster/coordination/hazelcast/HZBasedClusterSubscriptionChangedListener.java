 ...
 */
public class HZBasedClusterSubscriptionChangedListener implements MessageListener {

    private static Log log = LogFactory.getLog(HZBasedClusterSubscriptionChangedListener.class);

    /**
     * Listeners interested for subscription change notifications
     */
    private List<SubscriptionNotificationHandler> subscriptionListeners = new ArrayList<>(1);
     ...
     */
    public void addSubscriptionListener(SubscriptionNotificationHandler listener) {
    ...
    public void onMessage(Message message) {
        if (!(message.getPublishingMember().localMember())) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Handling cluster gossip: received a subscription change notification "
                        + clusterNotification.getEncodedObjectAsString());
            }
            for (SubscriptionNotificationHandler subscriptionListener : subscriptionListeners) {
                subscriptionListener.handleClusterNotification(clusterNotification);
            }
        }
    }
}
