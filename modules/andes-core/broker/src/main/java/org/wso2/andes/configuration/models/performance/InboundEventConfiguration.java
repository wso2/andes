...
/**
 * Configuration model for inbound events related configs
 */
@Configuration(description = "Inbound events related configurations")
public class InboundEventConfiguration {

    @Element(description = "Number of parallel writers used to write content to message store. Increasing this\n"
            + " value will speed-up the message receiving mechanism. But the load on the data store will increase")
    ...
    @Element(description = "Size of the Disruptor ring buffer for inbound event handling. For publishing at\n"
            ...
            + " memory and write.NOTE: Buffer size should be a value of power of two")
