...
public class AMQMessage implements ServerMessage
{
     ...
     *
     *
     * @throws org.wso2.andes.server.queue.MessageCleanupException when an attempt was made to remove the message from the message store and that
     *                                 failed
     */
    public void decrementReference()
    {
      ...
      /* Removing this else close since it is happening when same massages is tried to discard with two
      paths 1. SimpleAMQQueue - When ack received
       2. AMQ Channel - When unsubscribed
        else
        {
            if (count < 0)
            {
    ...
    {
        return getMessageMetaData().getMessagePublishInfo().getRoutingKey().toString();
    }
