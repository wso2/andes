...
//        MainRegistry.getVersionSpecificRegistry(ProtocolVersion.getLatestSupportedVersion());
    ...
    private void deliverMessageToAMQSession(int channelId, UnprocessedMessage msg)
    {
        ...
        if(session == null){
            //this happens if server sends us messages while we are closing the session. This can happen
            //if we are creating and deleting sessions in a tight loop (do not do that). 
            //All we can do is ignore the message and log
